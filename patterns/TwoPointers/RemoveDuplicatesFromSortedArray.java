 ...
 * 
 * ! EASY
 * #TwoPointers  #Arrays  #Fast&SlowPointers
 *
 * Given an integer array nums sorted in non-decreasing order, remove duplicates in-place
 * such that each unique element appears only once. Keep the relative order. Return the
 * number of unique elements in nums.
 *
 * Custom Judge: Test with code below:
...
public class RemoveDuplicatesFromSortedArray {
   ...
   public static int removeDuplicates(int[] nums) {
      ...
      int slow = 0;
      // int uniqueCount = 1; //simplify to use slow pointer to count unique elems
   ...
   public static void main(String[] args) {
      ...
      assertResult(nums1, expectedNums1, k1);
      ...
      assertResult(nums2, expectedNums2, k2);
      // Add more test cases if needed
   }

   public static void assertResult(int[] nums, int[] expectedNums, int k) {
      assert k == expectedNums.length;
